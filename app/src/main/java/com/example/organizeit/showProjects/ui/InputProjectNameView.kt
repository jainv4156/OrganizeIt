...
fun InputProjectNameView(){
    ...
    Box (
        ...
        Column {
            OutlinedTextField(value = viewModel.projectName.collectAsState().value, onValueChange ={viewModel.setProjectName(it)} )
            Row (modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween){
                ...
                Button(modifier = Modifier
                    .padding( 16.dp),onClick = { scope.launch {  viewModel.createProjectFolder()}}) {
                    Text(text = "Create")
                }
            }
        }
